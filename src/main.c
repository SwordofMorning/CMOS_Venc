...
#define SAMPLE_VENC_CHECK_NULLPTR(ptr) do { \
        if (ptr == VS_NULL) { \
...
static vs_int32_t sample_venc_lowlatency_param_parse(vs_int32_t argc, char *argv[],
    lowlatency_aquire_strm_type_e *p_aquire_strm_type, char **p_udp_host, vs_int32_t *p_udp_port)
{
    if (strstr(argv[argc-1], "-h") || argc < 4) {
...
vs_int32_t sample_venc_check_performance(vs_int32_t sensor_id, vs_bool_t *p_vpp_chn_enable)
{
    ...
    if (sensor_size.width >= PIC_4K_WIDTH && sensor_size.height >= PIC_4K_HEIGHT
        && sensor_framerate >= 60 && vpp_chn_num > 1) {
...
static vs_int32_t sample_venc_sys_init(vs_int32_t sensor_id, vs_size_s input_size)
{
    ...
    for (i = 0; i < g_pool_cnt; i++) {
        ...
        printf("index %d, blk_size[%llu] blk_cnt[%u]!\n", i,
            vb_cfg.ast_commpool[i].blk_size, vb_cfg.ast_commpool[i].blk_cnt);
    }
...
static vs_void_t sample_venc_stream_threadparam_get(sample_venc_acquire_stream_param_s *p_acquire_stream_param,
...
static vs_void_t sample_venc_udp_send(vs_int32_t socket_fd, struct sockaddr_in *socket_addr,
    vs_uint8_t *p_virt_addr, vs_uint32_t len)
{
...
static vs_void_t sample_venc_stream_callback(vs_int32_t chnid, vs_void_t *p_acquire_param, vs_venc_stream_s *p_out_stream)
{
    ...
    for (i = 0; i < p_out_stream->pack_num; i++) {
        ...
        for (j = 0; j < p_out_stream->p_pack[i].data_num; j++) {
            sample_venc_udp_send(p_udp_info->socket_fd, &p_udp_info->socket_addr,
                p_out_stream->p_pack[i].virt_addr + p_out_stream->p_pack[i].pack_info[j].pack_offset,
                p_out_stream->p_pack[i].pack_info[j].pack_len);
            wr_len += p_out_stream->p_pack[i].pack_info[j].pack_len;
        }
        sample_venc_udp_send(p_udp_info->socket_fd, &p_udp_info->socket_addr,
                p_out_stream->p_pack[i].virt_addr + p_out_stream->p_pack[i].offset,
